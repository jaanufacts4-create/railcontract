...
function compute(e: Partial<OBHSEntry>, t: OBHSTrain) {
  const F = e.ehk_present ?? 1, G = e.ac_short ?? 0, H = e.nac_short ?? 0, P = e.psi_pct ?? 0
  ...
  let psiPct = 0, psiLabel = '', psiPenalty = 0, staffPenalty = 0, janitorPenalty = 0, ehkPenalty = 0
  if (F === 0 && D === G && E === H) {
    psiLabel = '—'; staffPenalty = fullTrip / 2
  } else {
    if      (P < 50) { psiPct = 0;  psiLabel = 'No Pay'; psiPenalty = fullTrip / 2 }
    else if (P < 65) { psiPct = 20; psiLabel = '20%';    psiPenalty = fullTrip * 0.20 }
    else if (P < 75) { psiPct = 10; psiLabel = '10%';    psiPenalty = fullTrip * 0.10 }
    else if (P < 85) { psiPct = 5;  psiLabel = '5%';     psiPenalty = fullTrip * 0.05 }
    else             { psiPct = 0;  psiLabel = 'Nil' }
    if (G > 0 || H > 0) janitorPenalty = (G*L*N) + (H*L*N)
    if (F === 0)         ehkPenalty    = min_wages * 3
  }
  const otherPenalty = (e.w_penalty??0)+(e.x_penalty??0)+(e.aa_penalty??0)+(e.ab_penalty??0)+
    (e.ac_penalty??0)+(e.ad_penalty??0)+(e.ae_penalty??0)+(e.af_penalty??0)
  return { acHrs, nacHrs, psiPct, psiLabel, psiPenalty, staffPenalty,
           janitorPenalty, ehkPenalty, otherPenalty,
           totalPenalty: psiPenalty+staffPenalty+janitorPenalty+ehkPenalty+otherPenalty }
}

function fmt(n: number) { return n === 0 ? '—' : '₹'+n.toLocaleString('en-IN',{maximumFractionDigits:0}) }
...
function getRunDates(monthYear: string, daysJson: string): string[] {
  ...
  const all = Array.from({length: total}, (_,i) => `${monthYear}-${String(i+1).padStart(2,'0')}`)
  if (days.includes('Daily')) return all
  const allowed = new Set(days.map(d => DAY_IDX[d]))
  return all.filter(d => allowed.has(new Date(d+'T00:00:00').getDay()))
}
...
function TrainForm({ initial, onSave, onCancel, saving }: {
  ...
  const [f, setF] = useState({
    ...
    ehk_rate: initial?.ehk_rate??76.92, ac_rate: initial?.ac_rate??70.88, nac_rate: initial?.nac_rate??68.92,
  ...
  const inp = (label:string, key:string, type='number', step?:string) => (
    <div>
      ...
      <input className="input" type={type} step={step??'0.01'}
        value={(f as Record<string,string|number>)[key]} style={{width:'100%'}}
...
export default function OBHSScheduleModule({ apiBase, reportApi }: {
  apiBase: string      // e.g. '/api/obhs' or '/api/nirmal/obhs'
  reportApi: string    // e.g. '/api/obhs/report' or '/api/nirmal/obhs/report'
}) {
  ...
  async function loadTrains() {
    const r = await fetch(apiBase+'/trains'); setTrains(await r.json())
  }
  ...
  async function saveTrain(data: Partial<OBHSTrain>) {
    ...
    await loadTrains(); setTrainSaving(false)
  }

  async function deleteTrain(train_no: string) {
    if (!confirm(`Delete train ${train_no} and ALL its entries?`)) return
    await fetch(`${apiBase}/trains/${encodeURIComponent(train_no)}`,{method:'DELETE'})
    if (selected===train_no) setSelected(null); loadTrains()
  }

  function openAddForm() {
    setForm({...BLANK, date:monthYear+'-01'}); setEditingEntry(null); setShowForm(true); setPenOpen(false); setMsg('')
  }
  function openEditForm(e: OBHSEntry) {
    setForm({date:e.date,ehk_present:e.ehk_present,ac_short:e.ac_short,nac_short:e.nac_short,psi_pct:e.psi_pct,
      w_penalty:e.w_penalty,x_penalty:e.x_penalty,aa_penalty:e.aa_penalty,ab_penalty:e.ab_penalty,
      ac_penalty:e.ac_penalty,ad_penalty:e.ad_penalty,ae_penalty:e.ae_penalty,af_penalty:e.af_penalty})
    setEditingEntry(e); setShowForm(true); setPenOpen(false); setMsg('')
  }
  ...
  async function saveEntry() {
    ...
    const res = editingEntry
      ? await fetch(`${apiBase}/entries/${editingEntry.id}`,
  ...
  async function downloadReport() {
    setDownloading(true)
    try {
      const res = await fetch(`${reportApi}?month_year=${monthYear}`)
      if (!res.ok){alert('Error generating report');return}
      const blob = await res.blob()
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a'); a.href=url; a.download=`OBHS_Report_${monthYear}.xlsx`; a.click()
  ...
  const totals  = useMemo(()=>{
    ...
    return entries.reduce((acc,e)=>{
      ...
      return {acHrs:acc.acHrs+c.acHrs,nacHrs:acc.nacHrs+c.nacHrs,
        psiPenalty:acc.psiPenalty+c.psiPenalty,staffPenalty:acc.staffPenalty+c.staffPenalty,
        janitorPenalty:acc.janitorPenalty+c.janitorPenalty,ehkPenalty:acc.ehkPenalty+c.ehkPenalty,
        otherPenalty:acc.otherPenalty+c.otherPenalty,totalPenalty:acc.totalPenalty+c.totalPenalty}
            ...
            {trains.map(t=>(
              <div key={t.train_no} style={{display:'flex',alignItems:'center',gap:4}}>
                <button onClick={()=>{setSelected(t.train_no);setShowForm(false);setEditingTrain(null)}}
                  style={{flex:1,textAlign:'left',padding:'7px 10px',borderRadius:8,border:'none',cursor:'pointer',
                    fontFamily:'var(--font)',fontSize:13,fontWeight:selected===t.train_no?700:500,
                ...
                <button onClick={()=>{setEditingTrain(t);setShowAddTrain(false)}}
                  style={{background:'none',border:'none',cursor:'pointer',color:'var(--text-4)',padding:4}}><Pencil size={11}/></button>
                <button onClick={()=>deleteTrain(t.train_no)}
                  style={{background:'none',border:'none',cursor:'pointer',color:'var(--danger)',padding:4}}><Trash2 size={11}/></button>
              </div>
            ))}
          </div>
            ...
            {showForm&&(
              <div className="card" style={{padding:20}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
                  <p style={{fontSize:13,fontWeight:700,margin:0,color:'var(--text)'}}>{editingEntry?'Edit Entry':'New Entry'}</p>
                  <button onClick={()=>{setShowForm(false);setEditingEntry(null)}}
                    style={{background:'none',border:'none',cursor:'pointer',color:'var(--text-4)'}}><X size={16}/></button>
                </div>
                <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))',gap:12,marginBottom:16}}>
                      ...
                      return (
                        <select className="input" style={{width:'100%'}}
                          ...
                          {runDates.map(d=>{
                            ...
                            return <option key={d} value={d}>{d.slice(8)} ({DAY_SHORT[dt.getDay()]})</option>
                          })}
                        </select>
                      )
                    })()}
                  ...
                  <div><label style={{fontSize:11,color:'var(--text-3)',display:'block',marginBottom:3}}>AC Janitors Short</label>{NUM('ac_short')}</div>
                  <div><label style={{fontSize:11,color:'var(--text-3)',display:'block',marginBottom:3}}>NAC Janitors Short</label>{NUM('nac_short')}</div>
                  <div>
                    <label style={{fontSize:11,color:'var(--text-3)',display:'block',marginBottom:3}}>PSI % (0-100)</label>
                    <input type="number" min={0} max={100} step={0.01} className="input" style={{width:'100%'}}
                      value={form.psi_pct} onChange={e=>setForm(p=>({...p,psi_pct:Number(e.target.value)}))}/>
                  </div>
                </div>

                {preview&&(
                  <div style={{background:'var(--surface-2)',borderRadius:8,padding:'10px 14px',
                    display:'flex',gap:16,flexWrap:'wrap',marginBottom:14,border:'1px solid var(--border)'}}>
                    <span style={{fontSize:12,color:'var(--text-3)'}}>AC Hrs: <strong style={{color:'#3B82F6'}}>{fmtHrs(preview.acHrs)}</strong></span>
                    <span style={{fontSize:12,color:'var(--text-3)'}}>NAC Hrs: <strong style={{color:'#22C55E'}}>{fmtHrs(preview.nacHrs)}</strong></span>
                    <span style={{fontSize:12,color:'var(--text-3)'}}>PSI Slab: <strong style={{color:preview.psiLabel==='Nil'?'#16a34a':'#F59E0B'}}>{preview.psiLabel}</strong></span>
                    ...
                    <span style={{fontSize:12,fontWeight:700,color:'#DC2626',marginLeft:'auto'}}>Total: ₹{preview.totalPenalty.toFixed(0)}</span>
                  </div>
                )}

                <button onClick={()=>setPenOpen(o=>!o)}
                  ...
                  {penOpen?<ChevronUp size={14}/>:<ChevronDown size={14}/>} Manual Penalties (W-AF)
                </button>
                {penOpen&&(
                  <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:10,marginBottom:14,paddingBottom:14,borderBottom:'1px solid var(--border)'}}>
                    {[['w_penalty','W — Non-auth PSI coach'],['x_penalty','X — Without uniform'],
                      ['aa_penalty','AA — Liquid soap'],['ab_penalty','AB — Tissue paper roll'],
                      ['ac_penalty','AC — Deodorant cake'],['ad_penalty','AD — Room freshener'],
                      ['ae_penalty','AE — Biometric not provided'],['af_penalty','AF — Passenger complaints'],
                    ].map(([k,label])=>(
                      <div key={k}>
                        <label style={{fontSize:11,color:'var(--text-3)',display:'block',marginBottom:3}}>{label}</label>
