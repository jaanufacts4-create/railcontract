...
export async function GET(req: Request) {
  ...
  for (const date of dateRange(from, to)) {
    ...
    if (daySched > 0) dailySummary.push({ date, dow, sched: daySched, done: dayDone })
  }
  ...
  const trainSummary: TrainSumRow[] = []
  ...
  for (const s of schedule) {
    ...
    trainSummary.push({
      ...
      occurrences,
      totalAC:     s.ac_count  * occurrences,
      totalNAC:    s.nac_count * occurrences,
      grandTotal: (s.ac_count + s.nac_count) * occurrences,
    })
  }
  ...
  return NextResponse.json({
    from, to,
    totals: {
      totalSched:   statusRows.length,
      totalDone:    statusRows.filter(r => r.done).length,
      totalPending: statusRows.filter(r => !r.done).length,
    },
    statusRows, dailySummary, trainSummary,
  })
}
