...
export async function GET(req: NextRequest) {
  ...
  // Load all trains
  const { rows: trainRows } = await db.execute('SELECT * FROM nirmal_obhs_trains ORDER BY id')
  // Load all entries for the month
  const { rows: entryRows } = await db.execute({
    sql:  `SELECT * FROM nirmal_obhs_entries WHERE month_year = ? ORDER BY train_no, date`,
    args: [month_year],
  })
