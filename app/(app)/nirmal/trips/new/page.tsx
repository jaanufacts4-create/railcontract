...
export default function NirmalNewTripPage() {
  ...
  const [date,        setDate]        = useState(() => {
    ...
    if (m) {
      ...
      return m === curMonth ? new Date().toISOString().slice(0, 10) : `${m}-01`
    }
  ...
  const [intExtScores, setIntExtScores] = useState<Record<number, number>>({})
  ...
  useEffect(() => {
    function onClickOutside(e: MouseEvent) {
      if (trainInputRef.current && !trainInputRef.current.contains(e.target as Node)) setShowSuggestions(false)
      if (supInputRef.current   && !supInputRef.current.contains(e.target as Node))   setShowSupSuggestions(false)
    }
    document.addEventListener('mousedown', onClickOutside)
    return () => document.removeEventListener('mousedown', onClickOutside)
  }, [])

  const suggestions    = trainNo.length >= 2  ? allTrains.filter(t => t.startsWith(trainNo) && t !== trainNo).slice(0, 8) : []
  const supSuggestions = supervisor.length >= 2 ? allSupervisors.filter(s => s.toLowerCase().includes(supervisor.toLowerCase()) && s !== supervisor).slice(0, 6) : []

  type SchedWarn = { notInSchedule?: boolean; dayMismatch?: boolean; tripDay?: string; scheduledDays?: string[]; acMismatch?: boolean; nacMismatch?: boolean; schedAC?: number; schedNAC?: number; actualAC?: number; actualNAC?: number }
  const [schedWarn, setSchedWarn] = useState<SchedWarn | null>(null)

  function effType(pos: number, original: string): string { return compOverride[pos] ?? original }
  ...
  const acCount = acPositions.length; const nacCount = nacPositions.length; const intCount = intPositions.length
  ...
  function handleTypeChange(position: number, newType: string) {
    ...
    const wasInt = oldType === 'INT'; const isInt = newType === 'INT'
    ...
    if (!wasInt && isInt) {
      ...
      if (!isGEN) setCriteria(prev => prev[position] ? prev : { ...prev, [position]: [...DEFAULT_CRITERIA] as CriteriaRow })
    } else if (!wasInt && !isInt) {
      if (!wasGEN && isGEN) { setCriteria(prev => { const n = { ...prev }; delete n[position]; return n }) }
      else if (wasGEN && !isGEN && !criteria[position]) { setCriteria(prev => ({ ...prev, [position]: [...DEFAULT_CRITERIA] as CriteriaRow })) }
    }
  }
  ...
  async function pull(overrideTrainNo?: string) {
    ...
    if (!data.positions?.length) return setMsg(`Train ${t} not found in Train Master.`)
    const pos: Position[] = data.positions
    setPositions(pos); setCompOverride({}); setIntCriteria({}); setIntExtScores({}); setIntPrevType({})
    const c: Criteria = {}
    for (const { position, coach_type } of pos) {
      if (coachCategory(coach_type) !== 'GEN') c[position] = [...DEFAULT_CRITERIA] as CriteriaRow
    }
    setCriteria(c)
    ...
    try {
      ...
      if (!sched) { setSchedWarn({ notInSchedule: true }) } else {
        ...
        const acMismatch = sched.ac_count !== actualAC; const nacMismatch = sched.nac_count !== actualNAC
        if (!dayOk || acMismatch || nacMismatch) {
          setSchedWarn({ dayMismatch: !dayOk, tripDay, scheduledDays: sched.days, acMismatch, nacMismatch, schedAC: sched.ac_count, schedNAC: sched.nac_count, actualAC, actualNAC })
        } else setSchedWarn(null)
      }
  ...
  function setC(position: number, cIdx: number, val: number) {
    ...
    setCriteria(prev => { const row = [...(prev[position] ?? [...DEFAULT_CRITERIA])] as CriteriaRow; row[cIdx] = clamped; return { ...prev, [position]: row } })
  }
  function setIC(position: number, cIdx: number, val: number) {
    const clamped = Math.min(3, Math.max(0, val))
    setIntCriteria(prev => { const row = [...(prev[position] ?? [...INT_DEFAULT_CRITERIA])] as CriteriaRow; row[cIdx] = clamped; return { ...prev, [position]: row } })
  }
  ...
  async function submit() {
    ...
    for (const p of attendable) { const tot = scores[p.position] ?? 0; if (tot > 15) overLimit.push(`Coach ${p.position}: ${tot}/15`) }
    for (const p of intPositions) { const tot = calcTotal(intCriteria[p.position] ?? ([...INT_DEFAULT_CRITERIA] as CriteriaRow)); if (tot > 18) overLimit.push(`INT Coach ${p.position}: ${tot}/18`) }
    if (overLimit.length > 0) return setMsg(`⚠ Score over limit:\n${overLimit.join('\n')}`)
    ...
    const intensiveCoaches = intPositions.map(p => ({
      ...
      ext_score: intExtScores[p.position] ?? 3,
    }))

    const res = await fetch('/api/nirmal/trips', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        date, train_no: trainNo, wl_no: wlNo || null,
        supervisor, month_year: monthYear,
        scores,
    ...
    if (res.ok) {
      ...
      if (res.status === 409) alert(m); else setMsg(m)
    }
  }
  ...
  return (
    ...
    <div style={{ display: 'flex', gap: 4, marginBottom: 20, borderBottom: '2px solid var(--border)' }}>
      {([
        ...
        <button key={t.id} onClick={() => { setSubTab(t.id); if (t.id === 'wl') setWlDate(date) }} style={{
          ...
          color: subTab === t.id ? 'var(--accent)' : 'var(--text-3)', marginBottom: -2,
    ...
    {subTab === 'entry' && <div className="space-y-5">
      <h1 className="text-xl font-bold">New Trip Entry — Nirmal</h1>
        ...
        <Field label="Train No.">
          <div ref={trainInputRef} style={{ position: 'relative' }}>
            <input className="input" value={trainNo}
              ...
              placeholder="e.g. 14674" autoComplete="off" />
            {showSuggestions && suggestions.length > 0 && (
              <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50, background: 'var(--surface)', border: '1.5px solid var(--border-md)', borderRadius: 8, boxShadow: '0 4px 16px rgba(0,0,0,0.12)', marginTop: 2, overflow: 'hidden' }}>
                {suggestions.map(t => (
                  <div key={t} onMouseDown={() => { setTrainNo(t); setShowSuggestions(false); pull(t) }}
                    style={{ padding: '7px 12px', fontSize: 13, fontWeight: 600, cursor: 'pointer', color: 'var(--text)', borderBottom: '1px solid var(--border)' }}
        ...
        <Field label="Supervisor">
          <div ref={supInputRef} style={{ position: 'relative' }}>
            <input className="input" value={supervisor}
              ...
              onFocus={() => setShowSupSuggestions(true)} autoComplete="off" placeholder="Name" />
            {showSupSuggestions && supSuggestions.length > 0 && (
              <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50, background: 'var(--surface)', border: '1.5px solid var(--border-md)', borderRadius: 8, boxShadow: '0 4px 16px rgba(0,0,0,0.12)', marginTop: 2, overflow: 'hidden' }}>
                {supSuggestions.map(s => (
                  <div key={s} onMouseDown={() => { setSupervisor(s); setShowSupSuggestions(false) }}
                    style={{ padding: '7px 12px', fontSize: 13, fontWeight: 500, cursor: 'pointer', color: 'var(--text)', borderBottom: '1px solid var(--border)' }}
                    onMouseEnter={e => (e.currentTarget.style.background = 'var(--surface-2)')}
                    onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    👤 {s}
                  </div>
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => pull()} className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded font-medium text-sm">
          ⬇ Pull Data
        </button>
      ...
      {schedWarn && (
        <div style={{ padding: '12px 16px', borderRadius: 10, background: '#FEFCE8', border: '1.5px solid #FDE047', display: 'flex', flexDirection: 'column', gap: 5 }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: '#92400E', margin: 0 }}>⚠ Schedule Mismatch Detected</p>
          {schedWarn.notInSchedule && <p style={{ fontSize: 12, color: '#92400E', margin: 0 }}>Train {trainNo} is not found in Schedule of Trains.</p>}
          {schedWarn.dayMismatch && <p style={{ fontSize: 12, color: '#92400E', margin: 0 }}><b>{schedWarn.tripDay}</b> is not scheduled. Scheduled: <b>{schedWarn.scheduledDays?.join(', ')}</b></p>}
          {schedWarn.acMismatch && <p style={{ fontSize: 12, color: '#92400E', margin: 0 }}>AC mismatch — Schedule: <b>{schedWarn.schedAC}</b>, Actual: <b>{schedWarn.actualAC}</b></p>}
          {schedWarn.nacMismatch && <p style={{ fontSize: 12, color: '#92400E', margin: 0 }}>NAC mismatch — Schedule: <b>{schedWarn.schedNAC}</b>, Actual: <b>{schedWarn.actualNAC}</b></p>}
          <p style={{ fontSize: 10, color: '#A16207', margin: 0 }}>Warning only — you can still submit.</p>
        </div>
      )}
      ...
      {positions.length > 0 && (
        <div className="bg-white rounded-lg shadow p-3">
          <div className="flex items-center gap-3 mb-2 flex-wrap">
            ...
            {intCount > 0 && <span className="px-2 py-0.5 bg-purple-100 text-purple-700 rounded text-xs font-semibold">INT: {intCount}</span>}
          </div>
          <p className="text-xs text-gray-500 mb-2 font-medium">Normal Ratings — Total = (1X2 × 2) + row 2 + 3 + 4 + 5</p>
          <div className="overflow-x-auto">
            <table className="border-collapse text-xs">
              <thead>
                <tr>
                  <th className="proforma-label bg-yellow-100">Coach No.</th>
                  {positions.map(p => <th key={p.position} className="proforma-cell bg-yellow-100 font-bold text-gray-700 min-w-[36px]">{p.position}</th>)}
                </tr>
                <tr>
              ...
              <tbody>
                {CRITERIA_LABELS.map((label, cIdx) => (
                  <tr key={label} className={cIdx % 2 === 0 ? 'bg-blue-50' : 'bg-white'}>
                    ...
                    {positions.map(p => {
                      const eff = effType(p.position, p.coach_type)
                      const isInt = eff === 'INT'; const isAttendable = !isInt && coachCategory(eff) !== 'GEN'
                      const val = criteria[p.position]?.[cIdx] ?? (isAttendable ? DEFAULT_CRITERIA[cIdx] : 0)
                      return (
                        <td key={p.position} className="proforma-cell">
                          {isInt ? <span className="text-[9px] font-bold text-purple-400">INT</span>
                           : isAttendable ? (
                ...
                <tr className="bg-yellow-100 font-bold">
                  ...
                  {positions.map(p => {
                    const eff = effType(p.position, p.coach_type); const isInt = eff === 'INT'; const cat = coachCategory(eff)
                    const total = criteria[p.position] ? calcTotal(criteria[p.position]) : 0
                    return (
                      <td key={p.position} className={`proforma-cell font-bold text-sm ${isInt ? 'text-purple-400' : cat==='AC' ? 'text-blue-700' : cat==='NAC' ? 'text-green-700' : 'text-gray-300'}`}>
      ...
      {intPositions.length > 0 && (
        <div className="bg-white rounded-lg shadow p-3 border-2 border-purple-300">
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className="text-[10px] bg-purple-100 text-purple-600 px-2 py-0.5 rounded">Total = (1X2×2) + 2+3+4+5 + Ext</span>
          </div>
          <div className="overflow-x-auto">
            <table className="border-collapse text-xs">
              <thead>
                <tr>
                  ...
                  {intPositions.map(p => (
                    <th key={p.position} className="proforma-cell bg-purple-100 font-bold min-w-[40px] text-purple-800">
                      {p.position}<div className="text-[8px] font-normal text-purple-400">({intPrevType[p.position] ?? '—'})</div>
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                ...
                <tr className="bg-orange-50">
                  ...
                  {intPositions.map(p => (
                    <td key={p.position} className="proforma-cell">
                      <input type="number" min={0} max={3} value={intExtScores[p.position] ?? 3}
                ...
                <tr className="bg-yellow-100 font-bold">
                  ...
                  {intPositions.map(p => {
                    ...
                    const total = calcTotal(cr); const pct = Math.round(total / 18 * 100)
          ...
          <p className="text-[10px] text-gray-400 mt-2">Slab: ≥86% → Nil | 76–85% → 5% | 66–75% → 10% | 50–65% → 20% | &lt;50% → 100%</p>
        </div>
      )}
        ...
        <div className="bg-white rounded-lg shadow p-4 max-w-lg">
          ...
          <div className="flex items-center gap-10">
            ...
            <div>
              ...
              {deployed < mpRequired && deployed >= 0 && <p className="text-xs text-red-600 mt-1 font-medium">⚠ Short: {mpRequired - deployed} staff</p>}
              {deployed >= mpRequired && deployed > 0 && <p className="text-xs text-green-600 mt-1">✓ OK</p>}
            </div>
          </div>
        </div>
      )}
      ...
      {positions.length > 0 && (
        <div className="bg-white rounded-lg shadow p-4">
          <h2 className="font-semibold mb-3 text-sm text-gray-600">Annex A2 Penalties <span className="font-normal text-gray-400">(0 rakho agar nahi)</span></h2>
      ...
      {positions.length > 0 && (
        <div className="flex gap-4 items-center flex-wrap">
          ...
          {intCount > 0 && <span className="text-xs text-purple-600 bg-purple-50 border border-purple-200 rounded px-3 py-1.5">{intCount} INT coach{intCount > 1 ? 'es' : ''} → Intensive Summary</span>}
          <button onClick={() => router.back()} className="px-6 py-2 text-gray-500 hover:text-gray-700 text-sm">Cancel</button>
        </div>
      )}
    </div>}
