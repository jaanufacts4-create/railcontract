...
import { Plus, Download, Search, X, Train, LayoutList } from 'lucide-react'

type Trip = {
  id: number; date: string; train_no: string
  wl_no: string | null; supervisor: string; month_year: string
  ac_count: number; nac_count: number; int_count: number
}
...
export default function NirmalTripsPage() {
  ...
  useEffect(() => {
    setLoading(true)
    fetch(`/api/nirmal/trips?month_year=${monthYear}`).then(r => r.json()).then(d => { setTrips(d); setLoading(false) })
  }, [monthYear])
  ...
  function getTripFlags(t: Trip) {
    ...
    if (!sched.days.includes('Daily') && !sched.days.includes(tripDay))
      flags.push(`Day mismatch — ${tripDay} not scheduled`)
  ...
  async function del(id: number) {
    ...
    await fetch(`/api/nirmal/trips/${id}`, { method: 'DELETE' })
  ...
  const totals = visible.reduce((a, t) => ({ ac: a.ac + t.ac_count, nac: a.nac + t.nac_count }), { ac: 0, nac: 0 })

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--text)', margin: 0 }}>Trips — Nirmal</h1>
          <p style={{ fontSize: 13, color: 'var(--text-3)', margin: '3px 0 0' }}>M/s Nirmal Facility Management Service</p>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--surface)', border: '1.5px solid var(--border-md)', borderRadius: 10, padding: '7px 12px', minWidth: 210 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--surface)', border: '1.5px solid var(--border-md)', borderRadius: 10, padding: '7px 12px', minWidth: 180 }}>
      ...
      {loading && <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-4)', fontSize: 13 }}>Loading…</div>}
      ...
      {!loading && visible.length > 0 && (
        <div className="card" style={{ overflow: 'hidden', padding: 0 }}>
                  ...
                  <tr key={t.id}>
                    ...
                    <td>
                      <span style={{ fontWeight: 700, color: 'var(--text)', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                        <Train size={12} style={{ color: 'var(--text-4)' }} />{t.train_no}
                      </span>
                    </td>
                    <td style={{ color: 'var(--text-4)' }}>{t.wl_no ?? '—'}</td>
                    <td style={{ color: 'var(--text-2)' }}>{t.supervisor}</td>
                    <td>{t.ac_count > 0 ? <span className="badge badge-blue">{t.ac_count}</span> : <span style={{ color: 'var(--text-4)' }}>—</span>}</td>
                    <td>{t.nac_count > 0 ? <span className="badge badge-green">{t.nac_count}</span> : <span style={{ color: 'var(--text-4)' }}>—</span>}</td>
                    <td style={{ textAlign: 'center' }}>
                      {(() => {
                        const flags = getTripFlags(t)
                        return flags.length > 0 ? <span title={flags.join('\n')} style={{ cursor: 'help', fontSize: 14 }}>⚠️</span> : null
                      })()}
                    </td>
                    <td>
                      <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
                        <Link href={`/nirmal/trips/${t.id}/edit`}
                          style={{ fontSize: 12, fontWeight: 600, color: 'var(--primary)', textDecoration: 'none' }}>Edit</Link>
                        <button onClick={() => del(t.id)}
                          style={{ fontSize: 12, fontWeight: 600, color: 'var(--danger)', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>Delete</button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <div style={{ padding: '12px 20px', borderTop: '1px solid var(--border)' }}>
            <p style={{ fontSize: 12, color: 'var(--text-4)', margin: 0 }}>Showing {visible.length} of {trips.length} trips</p>
          </div>
        </div>
      )}
    </div>
