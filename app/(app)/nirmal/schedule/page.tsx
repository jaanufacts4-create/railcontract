...
import { Plus, Trash2, Train, CalendarDays, Pencil, Download } from 'lucide-react'
...
export default function NirmalSchedulePage() {
  ...
  async function load() {
    const data = await fetch('/api/nirmal/schedule').then(r => r.json())
    setTrains(data)
  }
  useEffect(() => { load() }, [])
  ...
  async function saveForm() {
    ...
    setSaving(true)
    await fetch('/api/nirmal/schedule', {
  ...
  async function del(train_no: string) {
    if (!confirm(`Remove ${train_no} from Nirmal schedule?`)) return
    await fetch(`/api/nirmal/schedule?train_no=${encodeURIComponent(train_no)}`, { method: 'DELETE' })
    load()
  }

  async function seedFromPrimary() {
    if (!confirm('Import all trains from Primary MCC schedule?\n\nExisting Nirmal trains will NOT be overwritten. You can then delete trains Nirmal doesn\'t handle.')) return
    setSeeding(true)
    const res  = await fetch('/api/nirmal/schedule', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ seed: true }) })
    const data = await res.json()
    setSeeding(false)
    setMsg(data.added > 0 ? `✓ Imported ${data.added} trains from Primary schedule` : 'All trains already present')
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0, height: '100%' }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--text)', margin: 0 }}>Schedule of Trains — Nirmal</h1>
        <p style={{ fontSize: 13, color: 'var(--text-3)', margin: '4px 0 0' }}>
          M/s Nirmal Facility Management Service — remove trains Nirmal doesn&apos;t handle
        </p>
      </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          {!editing && (
            <>
              ...
              <button onClick={seedFromPrimary} disabled={seeding} className="btn btn-secondary">
                <Download size={14} /> {seeding ? 'Importing…' : 'Import from Primary'}
              </button>
            </>
          )}
        ...
        <div className="card" style={{ overflow: 'hidden', padding: 0 }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <h2 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', margin: 0 }}>
              <CalendarDays size={14} style={{ display: 'inline', marginRight: 7, verticalAlign: 'middle' }} />
              Nirmal Scheduled Trains
            </h2>
          ...
          {trains.length === 0 ? (
            <div style={{ padding: 40, textAlign: 'center' }}>
              <p style={{ fontSize: 13, color: 'var(--text-4)', margin: '0 0 12px' }}>No trains yet.</p>
              <p style={{ fontSize: 12, color: 'var(--text-4)', margin: 0 }}>
                Click <strong>Import from Primary</strong> to copy the full schedule, then delete trains Nirmal doesn&apos;t handle.
              </p>
            </div>
          ) : (
