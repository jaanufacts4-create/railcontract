...
type ItemRow = {
  ...
  items_dirty: string
  penalty: string
  enabled: boolean   // ← togglable
}

function makeRow(name: string, enabled = true): ItemRow {
  return { item_name: name, lot_of: '', items_checked: '', items_dirty: '', penalty: '200', enabled }
}
...
export default function NewInspectionPage() {
  ...
  useEffect(() => {
    fetch('/api/inspections/inspectors')
      ...
      .catch(() => {})
  }, [savedMsg]) // refresh after each save
  ...
  function addRow() {
    // Find first default item not already in list, else 'Bed Sheet'
  ...
  async function handleSave() {
    ...
    setDate(nd)
    // Reset amounts but keep item names, inspector, designation
    setRows(prev => prev.map(r => ({ ...r, lot_of: '', items_checked: '', items_dirty: '' })))
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22, maxWidth: 920 }}>
      ...
      <div className="card" style={{ padding: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr 140px', gap: 14 }}>
          <div>
      ...
      <div className="card" style={{ padding: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          <div>
            ...
            <p style={{ fontSize: 11, color: 'var(--text-4)', margin: '3px 0 0' }}>Checkbox se item enable/disable karo</p>
          </div>
          ...
          <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: 700 }}>
            <thead>
              <tr>
                ...
                <th style={{ ...TH, minWidth: 160, textAlign: 'left' }}>Item Name</th>
            ...
            <tbody>
              {rows.map((row, i) => {
                ...
                const dim: React.CSSProperties = active ? {} : { opacity: 0.4, pointerEvents: 'none' }
                return (
                  <tr key={i} style={{ transition: 'opacity .15s' }}>
                    {/* Toggle checkbox */}
                    ...
                    <td style={{ ...TD, textAlign: 'center', fontWeight: 700, color: '#6B7280', fontSize: 11, background: rowBg }}>
                      {i + 1}
                    </td>
                    ...
                    <td style={{ ...TD, background: rowBg }}>
                      <div style={dim}>
                        <input type="number" min={0} style={{ ...inp, textAlign: 'right', minWidth: 90, borderColor: '#FECACA', color: '#DC2626' }}
                          value={row.penalty} onChange={e => setRow(i, 'penalty', e.target.value)} disabled={!active} />
                      </div>
                    </td>
            ...
            <tfoot>
              <tr>
                <td colSpan={7} style={{ padding: '7px 10px', fontWeight: 700, fontSize: 12, border: '1px solid #E5E7EB', background: '#F3F4F6', color: '#374151', textAlign: 'right' }}>
                  Total Penalty ({activeRows.length} items) →
                </td>
